...
def detect(save_img=False):
    ...
    GPIO.output(sigout,GPIO.HIGH)
    
    #pik = GPIO.input(signal)
    
    #while(True):
        #if GPIO.input(signal) != pik:
            #break
        #pik = GPIO.input(signal)
        #print('Wait')    
    #time.sleep(1)
    ...
    for order in range(12):
        ...
        while (True):
            str1 = ser.readline().decode()  # 获取arduino发送的数据
            if(str1 == "1\r\n"):
                print(str1 + 'Rec')
                break
            else:
                print(order)
        ser.close()
            #tk = (GPIO.input(signal))
            #print(order)
            #if tk == 1:
                #break
        
        GPIO.output(sigout,GPIO.LOW)
        
        while (True):
            cap = cv2.VideoCapture(0)
            ret,img = cap.read()
            img[:,:,1] = img[:,:,1] - np.ones((480,640)) * 18
            img = img[:,60:580,:]
...
def yolov5_main():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='best-leave.pt', help='model.pt path(s)')

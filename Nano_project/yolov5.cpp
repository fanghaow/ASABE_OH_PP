...
int main(int argc, char** argv) {
	...
	if (con != 0)fout.open("/home/dlinano/tensorrtx-master/yolov5/build/results/" + file_names[0] + ".txt");
        for (int b = 0; b < fcount; b++) {
            auto& res = batch_res[b];
            std::cout << res.size() << std::endl; //count zyf.2021/4/13
	    if (con != 0) fout << res.size() << std::endl;
            cv::Mat img = cv::imread(img_dir + "/" + file_names[f - fcount + 1 + b]);
            for (size_t j = 0; j < res.size(); j++) {
                cv::Rect r = get_rect(img, res[j].bbox);

		std::cout << r.tl() << "," << r.br() << std::to_string((int)res[j].class_id) << std::endl;
		if (con != 0) fout << r.tl() << "," << r.br() << std::to_string((int)res[j].class_id) << std::endl;

                cv::rectangle(img, r, cv::Scalar(0x27, 0xC1, 0x36), 2);
                cv::putText(img, std::to_string((int)res[j].class_id), cv::Point(r.x, r.y - 1), cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(0xFF, 0xFF, 0xFF), 2);
            }
            if (con != 0) cv::imwrite("_" + file_names[f - fcount + 1 + b], img);
	    std::cout << "_" + file_names[f - fcount + 1 + b] << std::endl;
	    if (con != 0) fout << "_" + file_names[f - fcount + 1 + b] << std::endl;
	    if (con != 0) fout << "Done" << std::endl;
        }
        fcount = 0;
	if (con != 0) fout.close();
